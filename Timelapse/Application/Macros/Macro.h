...
ref struct MacrosEnabled {
    static bool bMacroHP = false, bMacroMP = false, bMacroAttack = false, bMacroLoot = false;
};
...
ref class Macro {
    ...
    void SendKeyMacroToQueue() {
        ...
        case MacroType::HPPOTMACRO:
            if (MacrosEnabled::bMacroHP && HelperFuncs::IsInGame()) {
                if (String::IsNullOrWhiteSpace(Timelapse::MainForm::TheInstance->tbHP->Text))
                    break;
        ...
        case MacroType::MPPOTMACRO:
            if (MacrosEnabled::bMacroMP && HelperFuncs::IsInGame()) {
                if (String::IsNullOrWhiteSpace(Timelapse::MainForm::TheInstance->tbMP->Text))
                    break;
        ...
        case MacroType::LOOTMACRO:
            if (MacrosEnabled::bMacroLoot && HelperFuncs::ValidToLoot()) {
                if (String::IsNullOrWhiteSpace(Timelapse::MainForm::TheInstance->tbLootItemCount->Text))
                    break;
        ...
        case MacroType::ATTACKMACRO:
            if (MacrosEnabled::bMacroAttack && HelperFuncs::ValidToAttack()) {
                if (String::IsNullOrWhiteSpace(Timelapse::MainForm::TheInstance->tbAttackMobCount->Text))
                    break;
